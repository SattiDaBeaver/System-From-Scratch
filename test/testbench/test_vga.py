...
async def clear_buffers(dut):
    ...
    for i in range(600):
        dut.u_vga.buffer0[i].value = 0
        dut.u_vga.buffer1[i].value = 0
...
async def test_swap_applies_only_at_frame_start(dut):
    ...
    # V_COUNT_MAX=525 -- 700 cycles guarantees staying inside the same frame
    # since the design starts post-reset at h_count=v_count=0 and this many
    # cycles won't wrap V_COUNT_MAX*H_COUNT_MAX).
    await run_cycles(dut, 700)
    ...
    # Run past a full frame (800*525 cycles) so frame_start fires.
    await run_cycles(dut, 800 * 525)
...
async def test_hsync_vsync_pulse_widths(dut):
    ...
    frame -- matches the module's own front/back-porch constants."""
    cocotb.start_soon(Clock(dut.clk, 10, unit="ns").start())
    await reset(dut)
    await clear_buffers(dut)
    ...
    for _ in range(96):
        ...
        if int(dut.hsync.value) == 0:
            low_count += 1
    assert low_count == 96, f"hsync low for {low_count}/96 expected cycles"

    high = int(dut.hsync.value)
    await RisingEdge(dut.clk)
    assert int(dut.hsync.value) == 1 or high == 1, "hsync did not rise after the sync pulse window"
